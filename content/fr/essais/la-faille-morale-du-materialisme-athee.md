...
$$
\underbrace{\text{Is}(p)}_{\text{Fait : }p\text{ est vrai}}
$$
on ne peut en déduire:
$$
\text{Ought}(q)
$$
sans introduire une prémisse normative intermédiaire :
> $$
> \text{Is}(p) ,\  (\text{Is}(p) \to \text{Ought}(q)) \ \models\  \text{Ought}(q).
> $$
> {{< center-italic-text >}} Si \(p\) est vrai, alors \(q\) doit être fait, donc \(q\) doit être fait. {{< /center-italic-text >}}
...
L’utilitarisme définit le critère :
$$
\text{Maximiser } U = \sum_{i=1}^n u_i
$$
avec \\(u_i\\) le bonheur des individus. Mais **pourquoi maximiser \\(U\\)** ? Cette obligation n’est pas déduite des faits : elle est présupposée.
