...
Formally, if we assert:
$$
\underbrace{\text{Is}(p)}_{\text{Fact: }p\text{ is true}}
$$
we cannot deduce:
$$
\text{Ought}(q)
$$
without introducing an intermediate normative premise:
> $$
> \text{Is}(p) ,\  (\text{Is}(p) \to \text{Ought}(q)) \ \models\  \text{Ought}(q).
> $$
> {{< center-italic-text >}} If \(p\) is true, then \(q\) ought to be done, therefore \(q\) ought to be done. {{< /center-italic-text >}}
...
Utilitarianism defines the criterion:
$$
\text{Maximize } U = \sum_{i=1}^n u_i
$$
with \\(u_i\\) as the happiness of individuals. But **why maximize \\(U\\)?** This obligation is not deduced from facts: it is presupposed.
...
$$
without a prior ethical postulate.
